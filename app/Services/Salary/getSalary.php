    ...
    public function handle()
    {
        ...
        try {
            ...
            $fieldSales = [
                ...
                "unpaidLeaveDays" => ["name" => "Số ngày nghỉ không lương", "subTitle" => null, "type" => 0],
                "paidLeaveDays" => ["name" => "Số ngày nghỉ có lương", "subTitle" => null, "type" => 0],
                ...
                "responseContent" => ["name" => "Nộp bù", "subTitle" => null, "type" => 2],
            ...
            foreach ($salary as $record) {
                ...
                foreach ($employee->departmentId == 1 ? $fieldSales : $fields as $field => $info) {
                    $rowData[] = [
                        ...
                        "number" => strval($formattedNumber($record->$field)) ?? null,
                        "type" => $info["type"],
                    ];
                }

                $recordDate = Carbon::createFromFormat('Y-m-d', $record->createdAt);
                $comparisonDate = Carbon::create(2024, 5, 1);
                $previousMonthDate = $record->createdAt;
                if ($recordDate->greaterThan($comparisonDate)){
                    $previousMonthDate = $recordDate->subMonth()->format('Y-m-d');
                }
